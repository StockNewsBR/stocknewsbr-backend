...
def _get_provider_metrics() -> dict:
    provider_metrics = {}
...
def _get_provider_symbol_metrics() -> dict:
...
def _get_signal_quality_metrics() -> dict:
    return {
        source: dict(entry)
...
def _get_repeated_failures_metrics() -> list:
    return sorted(
        (
            {
                "symbol": symbol,
...
def get_performance_metrics_snapshot():
    with _lock:
        return {
            "http_endpoint_latency_seconds": _get_http_metrics(),
            "cache": _get_cache_metrics(),
            "external_provider_call_total": _get_provider_metrics(),
            "external_provider_symbol_call_total": _get_provider_symbol_metrics(),
            "worker_stage_seconds": _get_worker_metrics(),
            "signal_quality_coverage": _get_signal_quality_metrics(),
            "provider_symbol_failures": _get_repeated_failures_metrics(),
        }
