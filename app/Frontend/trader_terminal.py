...
function renderWorkspace() {{
  ...
  const panels = {{
    "home": renderHome(),
    "flow": renderAiToolList("Flow IA", WORKSPACE?.ai_tools?.flow || [], "Fluxo institucional, agressao e pressao compradora/vendedora."),
    "radar": renderHome(),
    "liquidity": renderAiToolList("Liquidity IA", WORKSPACE?.ai_tools?.liquidity || [], "Zonas de liquidez, sweeps, traps e invalidacao."),
    "trend": renderAiToolList("Trend IA", WORKSPACE?.ai_tools?.trend || [], "Direcao predominante e estrutura de mercado."),
    "momentum": renderAiToolList("Momentum IA", WORKSPACE?.ai_tools?.momentum || [], "Aceleracao, forca e exaustao."),
    ...
    "risk": renderAiToolList("Risk IA", WORKSPACE?.ai_tools?.risk || [], "Risco operacional, bloqueios, Can Trade e No Trade Reason."),
    "news-ia": renderAiToolList("News IA", WORKSPACE?.ai_tools?.news || [], "Estado da noticia, relevancia, confianca, impacto e provider status."),
