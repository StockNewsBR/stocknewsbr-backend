...
_CRYPTO_DISPLAY_UNIVERSE = {
    ticker.replace("-USD", "USD") for ticker in _CRYPTO_UNIVERSE
}
_PROFILE_AI_RULES = {
    "b3_stock": {
...
def _coherence_context_row(
    *,
    ...
    return {
        ...
        "breakout_probability_state": breakout_state
        or ai_bias.get("breakout_probability_state"),
...
def _event_copy(
    event_type: str, reason: str | None, coherence: Dict[str, Any]
) -> Dict[str, Any]:
    event_key = str(event_type or "").upper()
    base = _EVENT_COPY.get(
        event_key,
        {
            "label": event_key or "Watch",
            "direction": "observar",
            "note": "Observar",
            "confirmation": "",
        },
    ...
    reason_text = _REASON_COPY.get(
        reason_key, reason_key or "Evento tecnico do grafico."
    )
    trigger = coherence.get("trigger") or base.get("confirmation")
    invalidation = (
        coherence.get("invalidation")
        or "Invalidar se preco, volume, regime ou fluxo negarem a tese."
    )
    risk = (
        coherence.get("risk")
        or "Risco operacional depende de confirmacao de volume, tendencia e liquidez."
    )
    return {
        "label": base["label"],
...
def _build_ai_bias(ai_context: Dict[str, Any] | None, profile: str) -> Dict[str, Any]:
    ...
    if breakout_score >= 75:
        ...
        short_bonus += 1
    if (
        volatility_squeeze_state in {"squeeze_ready", "compression"}
        and smart_money_score >= 60
    ...
    if (
        regime_is_high_vol
        and master_score_value < profile_rules["high_vol_block_master_score"]
    ):
        long_block = True
        short_block = True
    exit_long_on_ai = regime_is_bear and (
        master_score_value < 60 or smart_money_score < 45
    )
    exit_short_on_ai = regime_is_bull and (
        master_score_value < 60 or smart_money_score >= 60
    )
    return {
...
def _infer_profile(symbol: str) -> str:
    ...
    if (
        ...
        return "bdr"
    if (
        normalized in _B3_UNIVERSE
        or display in _B3_DISPLAY_UNIVERSE
        or normalized.endswith(".SA")
    ):
        return "b3_stock"
    return "us_stock"
...
def _build_indicator_frame(
    ...
    df["vwap"] = (
        (cumulative_value / cumulative_volume.where(cumulative_volume > 0))
        .ffill()
        .fillna(df["ema21"])
    )
    ...
    df["volume_avg"] = volume.rolling(
        volume_lookback, min_periods=max(5, volume_lookback // 3)
    ).mean()
    df["resistance"] = (
        high.shift(1)
        .rolling(
    ...
    df["slope21"] = (df["ema21"] - df["ema21"].shift(slope_lookback)) / max(
        1, slope_lookback
    )
    return df
...
def _is_session_close_bar(time_value: Any, timeframe: str, profile: str) -> bool:
    ...
    if isinstance(time_value, str) and time_value.strip():
        ...
        has_explicit_tz = raw.endswith("Z") or any(
            token in raw[10:] for token in ("+", "-")
        )
...
def _effective_indicator_settings(
    settings: Dict[str, Any], bar_count: int
) -> Dict[str, int]:
    return {
        "breakout_lookback": min(
            int(settings.get("breakout_lookback", 12)), max(6, bar_count // 3)
        ),
        "atr_period": min(int(settings.get("atr_period", 14)), max(7, bar_count // 3)),
        "slope_lookback": min(
            int(settings.get("slope_lookback", 3)), max(2, bar_count // 12)
        ),
        "volume_lookback": min(
            int(settings.get("volume_lookback", 20)), max(8, bar_count // 2)
        ),
    }
...
class _TrendBreakoutEvaluator:
    def __init__(self, profile, settings, df, ai_bias, timeframe, display):
        ...
        self.events = []
        self.breakout_long_bar = -1
        self.breakout_short_bar = -1
        self.pullback_long_armed = False
        self.pullback_short_armed = False
        self.current_position = None
        self.entry_index = -1
        self.entry_price = 0.0
        self.best_price_after_entry = 0.0
        self.worst_price_after_entry = 0.0
        self.last_exit_index = -999
        self.latest_score = 0
        self.latest_signal = "NEUTRAL"
        self.latest_breakout = False
        self.latest_pullback = False
        self.latest_trend = "sideways"
        self.latest_coherence = None
        self.long_confidence = 0.0
        self.short_confidence = 0.0
        self.score_long = 0
        self.score_short = 0
        self.chart_regime_state = "unknown"
        self.liquidity_event = None
        self.coherence_row = None

    def evaluate_all(self, warmup):
        for index in range(len(self.df)):
            if index < warmup:
                continue
            self._process_bar(index)

    def _process_bar(self, index):
        df = self.df
        ...
        volume_ok = volume_rel >= settings["volume_factor"] or (
            not volume_known and price_expansion
        )
        volume_pullback_ok = ((volume < volume_avg) and (volume < volume_prev)) or (
            not volume_known and body_atr < settings["body_atr_min"]
        )
        volume_resume_ok = ((volume > volume_avg) and (volume > volume_prev)) or (
            not volume_known and price_expansion
        )
        candle_forte_long = (close > open_price) and (
            body_atr >= settings["body_atr_min"]
        )
        candle_forte_short = (close < open_price) and (
            body_atr >= settings["body_atr_min"]
        )
        wick_ok_long = (body > 0) and (
            upper_wick <= (body * settings["wick_max_ratio"])
        )
        wick_ok_short = (body > 0) and (
            lower_wick <= (body * settings["wick_max_ratio"])
        )
        dist_ema_ideal_long = abs(close - ema_mid) <= (
            atr_value * settings["ema_distance_atr"]
        )
        ...
        buying_into_resistance = (
            ...
            and ((resistance - close) / atr_value)
            < settings.get("resistance_room_atr", 0.4)
        )
        ...
        self.chart_regime_state = _classify_chart_regime(
        ...
        self.liquidity_event = _detect_liquidity_event(
        ...
        if (
            ...
            and self.liquidity_event not in {"sweep_high_reject", "supply_absorption"}
        ):
            self.chart_regime_state = "reversal_up"
        if (
            support_reject_short
            and not short_breakout
            and self.liquidity_event not in {"sweep_low_reclaim", "demand_absorption"}
        ):
            self.chart_regime_state = "reversal_down"
        if long_breakout:
            self.breakout_long_bar = index
            self.pullback_long_armed = False
        if short_breakout:
            self.breakout_short_bar = index
            self.pullback_short_armed = False
        breakout_recente_long = (
            self.breakout_long_bar >= 0
            and (index - self.breakout_long_bar) <= settings["pullback_max_bars"]
        )
        breakout_recente_short = (
            self.breakout_short_bar >= 0
            and (index - self.breakout_short_bar) <= settings["pullback_max_bars"]
        )
        ...
        if pullback_fraco_long:
            self.pullback_long_armed = True
        if pullback_fraco_short:
            self.pullback_short_armed = True
        if not breakout_recente_long:
            self.pullback_long_armed = False
        if not breakout_recente_short:
            self.pullback_short_armed = False
        long_pullback = (
            self.pullback_long_armed
            and long_trend
            and candle_forte_long
            and wick_ok_long
        ...
        continuation_volume_ok = volume_rel >= max(
            0.90, settings["volume_factor"] - 0.12
        ) or (not volume_known and price_expansion)
        long_continuation = (
            long_trend
            and volatility_ok
        ...
        if body_ok:
            self.score_long += 1
        if self.liquidity_event in {"sweep_low_reclaim", "demand_absorption"}:
            self.score_long += 1
        if short_trend:
            self.score_short += 3
        if volatility_ok:
        ...
        if body_ok:
            self.score_short += 1
        if self.liquidity_event in {"sweep_high_reject", "supply_absorption"}:
            self.score_short += 1
        long_liquidity_reversal = (
            self.liquidity_event in {"sweep_low_reclaim", "demand_absorption"}
            and close > ema_fast
            and close >= _safe_float(prev_row["close"])
            and self.score_long >= max(5, settings["score_min"] - 1)
        )
        short_liquidity_reversal = (
            self.liquidity_event in {"sweep_high_reject", "supply_absorption"}
            and close < ema_fast
            and close <= _safe_float(prev_row["close"])
            and self.score_short >= max(5, settings["score_min"] - 1)
        )
        long_trend_acceptance = (
            self.chart_regime_state == "trend_up"
        ...
        short_trend_acceptance = (
            self.chart_regime_state == "trend_down"
        ...
        self.score_long += int(ai_bias["long_bonus"])
        self.score_short += int(ai_bias["short_bonus"])
        required_score_long = max(
            5, settings["score_min"] + int(ai_bias["threshold_adjust"])
        )
        required_score_short = max(
            5, settings["score_min"] + int(ai_bias["threshold_adjust"])
        )
        trend_strength_score = max(
            0.0,
            min(
                100.0, spread_atr * 34.0 + (22.0 if long_trend or short_trend else 0.0)
            ),
        )
        breakout_state = (
            "ready_to_break"
            if (long_breakout or short_breakout)
                ...
                "building_pressure"
                if (breakout_recente_long or breakout_recente_short)
                else ai_bias.get("breakout_probability_state")
            )
        )
        self.coherence_row = _coherence_context_row(
            display=display,
            ai_bias=ai_bias,
            close=close,
            ema_mid=vwap,
            ...
            chart_regime_state=self.chart_regime_state,
            liquidity_event=self.liquidity_event,
        )
        long_entry = False
        short_entry = False
        event_reason = None
        self.long_confidence = _setup_confidence(
            side="long",
            score_side=self.score_long,
            score_opposite=self.score_short,
        ...
        self.short_confidence = _setup_confidence(
            side="short",
            score_side=self.score_short,
            score_opposite=self.score_long,
        ...
        if not volume_known:
            self.long_confidence = max(5.0, self.long_confidence - 6.0)
            self.short_confidence = max(5.0, self.short_confidence - 6.0)
        if self.current_position is None:
            cooldown_bars = int(settings.get("cooldown_bars_after_exit", 2))
            in_cooldown = (index - self.last_exit_index) < cooldown_bars
            strong_long_override = volume_rel >= (
                settings["volume_factor"] + 0.35
            ) and self.score_long >= (self.score_short + settings["min_score_edge"] + 3)
            strong_short_override = volume_rel >= (
                settings["volume_factor"] + 0.35
            ) and self.score_short >= (self.score_long + settings["min_score_edge"] + 3)
            if (
                not ai_bias["long_block"]
                and (not in_cooldown or strong_long_override)
                and long_pullback
                and not buying_into_resistance
                and self.score_long >= required_score_long
                and self.score_long >= (self.score_short + settings["min_score_edge"])
            ):
                long_entry = True
                ...
                and short_pullback
                and not selling_into_support
                and self.score_short >= required_score_short
                and self.score_short >= (self.score_long + settings["min_score_edge"])
            ):
                short_entry = True
                event_reason = "pullback_resume"
            elif (
                ...
                and self.long_confidence >= 60
                and self.score_long >= max(5, required_score_long - 1)
                and self.score_long >= (self.score_short + settings["min_score_edge"])
            ):
                long_entry = True
                ...
                and self.short_confidence >= 60
                and self.score_short >= max(5, required_score_short - 1)
                and self.score_short >= (self.score_long + settings["min_score_edge"])
            ):
                short_entry = True
                ...
                and self.long_confidence >= 60
                and (profile != "bdr" or (volume_ok and self.long_confidence >= 72))
                and self.score_long >= required_score_long
                and self.score_long >= (self.score_short + settings["min_score_edge"])
            ):
                long_entry = True
                ...
                and self.short_confidence >= 60
                and (profile != "bdr" or (volume_ok and self.short_confidence >= 72))
                and self.score_short >= required_score_short
                and self.score_short >= (self.score_long + settings["min_score_edge"])
            ):
                short_entry = True
                ...
                event_reason = "trend_continuation"
        event_time = row["time"]
        long_coherence = evaluate_trade_coherence(
            self.coherence_row, "BUY", bullish=self.score_long, bearish=self.score_short
        )
        short_coherence = evaluate_trade_coherence(
            self.coherence_row,
            "SHORT",
            bullish=self.score_long,
            bearish=self.score_short,
        )
        if long_entry and long_coherence["coherence_status"] == "blocked":
            long_entry = False
            self.latest_coherence = long_coherence
        if short_entry and short_coherence["coherence_status"] == "blocked":
            short_entry = False
            self.latest_coherence = short_coherence
        if long_entry:
            self.events.append(
                _event_payload(
                    event_type="BUY",
            ...
            self.best_price_after_entry = high
            self.worst_price_after_entry = low
            ...
            self.events.append(
                _event_payload(
                    ...
                    price=close,
                    time_value=event_time,
                    score=self.score_short,
                    reason=event_reason,
                    coherence=short_coherence,
                    confidence=self.short_confidence,
                    chart_regime_state=self.chart_regime_state,
                    liquidity_event=self.liquidity_event,
                )
            )
            ...
            self.best_price_after_entry = high
            self.worst_price_after_entry = low
            self.latest_signal = "SHORT"
            self.latest_coherence = short_coherence
        elif self.current_position == "long":
            self.best_price_after_entry = max(self.best_price_after_entry, high, close)
            self.worst_price_after_entry = min(self.worst_price_after_entry, low, close)
            profit_atr = (
                (self.best_price_after_entry - self.entry_price) / atr_value
            ...
            long_continuation_intact = (
                ...
                and close >= min(ema_mid, vwap)
                and self.score_long >= (self.score_short + settings["min_score_edge"])
                and self.liquidity_event
                not in {"sweep_high_reject", "supply_absorption"}
            )
            protect_profit = (
                profit_atr >= 1.35
                and close < ema_fast
                and close < _safe_float(prev_row["low"])
                and self.score_short >= (self.score_long - 1)
            ...
            time_stop = (
                (index - self.entry_index) >= settings["max_hold_bars"]
            ) and not long_continuation_intact
            should_exit_long = (
                time_stop
            ...
            if should_exit_long:
                ...
                sell_coherence = evaluate_trade_coherence(
                    self.coherence_row,
                    "SELL",
                    bullish=self.score_long,
                    bearish=self.score_short,
                ...
                self.events.append(
                    _event_payload(
                        ...
                        score=(
                            ...
                            else self.score_long
                        ),
                        reason=reason,
                        coherence=sell_coherence,
                        confidence=(
                            self.short_confidence
                            if reason == "opposite_signal"
                            else self.long_confidence
                        ),
                        chart_regime_state=self.chart_regime_state,
                        liquidity_event=self.liquidity_event,
                    )
                ...
                self.best_price_after_entry = 0.0
                self.worst_price_after_entry = 0.0
                self.last_exit_index = index
                self.latest_signal = "SELL"
                self.latest_coherence = sell_coherence
        elif self.current_position == "short":
            self.best_price_after_entry = max(self.best_price_after_entry, high, close)
            self.worst_price_after_entry = min(self.worst_price_after_entry, low, close)
            profit_atr = (
                (self.entry_price - self.worst_price_after_entry) / atr_value
                if self.entry_price > 0
                else 0.0
            )
            short_continuation_intact = (
                ...
                and self.score_short >= (self.score_long + settings["min_score_edge"])
                and self.liquidity_event
                not in {"sweep_low_reclaim", "demand_absorption"}
            )
            protect_profit = (
                profit_atr >= 1.35
                and close > ema_fast
                and close > _safe_float(prev_row["high"])
                and self.score_long >= (self.score_short - 1)
            ...
            if should_exit_short:
                ...
                if ai_bias["exit_short_on_ai"]:
                    ...
                    reason = "micro_structure_loss"
                cover_coherence = evaluate_trade_coherence(
                    self.coherence_row,
                    "COVER",
                    bullish=self.score_long,
                    bearish=self.score_short,
                ...
                self.worst_price_after_entry = 0.0
                self.last_exit_index = index
                self.latest_signal = "COVER"
                self.latest_coherence = cover_coherence
        self.latest_score = max(self.score_long, self.score_short)
        self.latest_breakout = bool(long_breakout or short_breakout)
        self.latest_pullback = bool(long_pullback or short_pullback)
        if long_trend:
        ...
        if self.latest_signal == "NEUTRAL":
            if self.current_position == "long":
                ...
                self.latest_signal = "WATCH_SHORT"
        if self.latest_coherence is None:
            watch_action = "BUY" if self.score_long >= self.score_short else "SHORT"
            self.latest_coherence = evaluate_trade_coherence(
                self.coherence_row,
                watch_action,
                bullish=self.score_long,
                bearish=self.score_short,
...
def build_trend_breakout_payload(
    ...
    evaluator = _TrendBreakoutEvaluator(
        profile, settings, df, ai_bias, timeframe, display
    )
    evaluator.evaluate_all(warmup)
    if evaluator.current_position in {"long", "short"} and _is_session_close_bar(
        df.iloc[-1]["time"], timeframe, profile
    ):
        final_row = df.iloc[-1]
        final_close = _safe_float(final_row["close"])
        if final_close > 0:
            if evaluator.current_position == "long":
                exit_type = "SELL"
                exit_score = evaluator.score_long
                exit_confidence = evaluator.long_confidence
            else:
                exit_type = "COVER"
                exit_score = evaluator.score_short
                exit_confidence = evaluator.short_confidence
            close_coherence = evaluate_trade_coherence(
                evaluator.coherence_row,
                exit_type,
                bullish=evaluator.score_long,
                bearish=evaluator.score_short,
            )
            evaluator.events.append(
                _event_payload(
                    ...
                    chart_regime_state=evaluator.chart_regime_state,
                    liquidity_event=evaluator.liquidity_event,
                )
            )
            evaluator.current_position = None
            evaluator.latest_signal = exit_type
            evaluator.latest_coherence = close_coherence
    ...
    return {
        ...
        "timeframe": timeframe,
        "profile": profile,
        "signal": evaluator.latest_signal,
        "score": round(evaluator.latest_score, 2),
        "trend": evaluator.latest_trend,
        "breakout": evaluator.latest_breakout,
        "pullback": evaluator.latest_pullback,
        "events": evaluator.events[-50:],
        "latest_event": evaluator.events[-1] if evaluator.events else None,
        "context": {
            ...
            "chart_regime_state": evaluator.chart_regime_state if True else "unknown",
            "liquidity_event": evaluator.liquidity_event if True else "none",
            "long_confidence": round(evaluator.long_confidence, 1) if True else 0.0,
            "short_confidence": round(evaluator.short_confidence, 1) if True else 0.0,
            "ai_bias": {
            ...
            "trade_coherence": evaluator.latest_coherence or {},
        },
    }
