...
def _build_series(ohlc: list, close_prices: list, ema9: list, ema21: list, ema50: list) -> list:
    series = []
...
def _process_signals(ticker: str, signals: list) -> Tuple[list, int, int, str]:
    markers = []
    bullish_markers = 0
    bearish_markers = 0
...
def build_chart_overlays(ticker: str, ohlc: list, signals: list) -> dict:
    ...
    markers, bullish_markers, bearish_markers, latest_signal = _process_signals(ticker, signals)
    zones = _build_zones(high_prices, low_prices)
    summary = _build_summary(
        ticker, close_prices, ema9, ema21, ema50, latest_signal, bullish_markers, bearish_markers, len(markers)
    )
