...
def _parse_signals(ticker: str, signals: list) -> Tuple[list, int, int, str]:
    markers = []
    bullish_markers = 0
    bearish_markers = 0
...
def _calculate_zones(high_prices: list, low_prices: list) -> list:
...
def build_chart_overlays(ticker: str, ohlc: list, signals: list):
    ...
    markers, bullish_markers, bearish_markers, latest_signal = _parse_signals(
        ticker, signals
    )

    zones = _calculate_zones(high_prices, low_prices)
