...
Contrato preservado: **todas as ações mutáveis exigem auth server-side** (`require_any_channel_access`/`require_active_plan`); ordem autenticar → autorizar → Social Guardian (`can_publish`/`validate_attachment_url`) → persistir → auditar. Guardian segue bloqueando links, e-mails, telefones, WhatsApp/Telegram, apostas/cassinos. Ownership em delete. **Autenticação do WebSocket de chat:** aceita cookie de sessão httpOnly (browser same-origin) e `Authorization: Bearer` quando o contrato do cliente o suporta; bearer token em query parameter ou na URL é **rejeitado**. Credenciais reutilizáveis não trafegam em URLs, histórico, logs ou referrers.
